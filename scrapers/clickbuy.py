...
class ClickbuyScraper(BaseScraper):
    """Scraper for Clickbuy (https://clickbuy.com.vn)"""
    ...
    def extract_product_info(self, page: Page, query: str, max_products: int) -> List[Product]:
        products = []
        try:
            # Cập nhật selector chuẩn xác theo cấu trúc HTML thực tế của Clickbuy
            product_elements = page.query_selector_all("div.list-products__item, .list-products_item")
            ...
            for element in product_elements[:max_products]:
                try:
                    # 1. Lấy tên sản phẩm từ strong.title_name hoặc attribute title của thẻ a
                    name_el = element.query_selector("strong.title_name, .title_name")
                    name = ""
                    if name_el:
                    ...
                    # 2. Lấy giá sản phẩm từ ins.new-price (chỉ lấy giá mới)
                    price_el = element.query_selector("ins.new-price")
    ...
    def crawl_all_phones(self, max_products: Optional[int] = None) -> List[Product]:
        """
        Cào TẤT CẢ sản phẩm điện thoại từ trang danh mục của Clickbuy.
        ...
        try:
            ...
            # Click "Xem thêm sản phẩm" liên tục cho đến hết
            logger.info("Đang tiến hành click nút 'Xem thêm sản phẩm'...")
            load_more_selector = "a.btn-show-more"

            for _ in range(30): # max 30 clicks
                try:
            ...
            logger.info("Đã tải xong toàn bộ danh sách sản phẩm trên trang.")
    ...
    def extract_comments(self, page: Page, product_url: str) -> List[str]:
        ...
        try:
            ...
            if not safe_goto(page, product_url, timeout=60000):
                return comments
            ...
            for _ in range(20): # max 20 clicks
                try:
                    ...
                    if btn.count() > 0 and btn.first.is_visible():
                        btn.first.click()
                        page.wait_for_timeout(1500)  # Chờ load thêm nội dung
                    else:
                        break
                except Exception:
                    break
            ...
            comment_elements = page.locator(".comments-content .comments-list .item-content").all()
    ...
    def scrape_price_from_url(self, product_url: str) -> Optional[Product]:
        """
        Cào tên và giá từ một trang sản phẩm cụ thể.
        Phương thức này được thiết kế để chạy độc lập, tự quản lý page.
        """
        page = self.browser_manager.new_page()
        try:
            ...
            page.wait_for_selector("h1.title-product, .price-new-ins", timeout=15000)

            name_el = page.query_selector("h1.title-product")
            name = name_el.inner_text().strip() if name_el else ""

            price_el = page.query_selector(".price-new-ins")
            price = price_el.inner_text().strip() if price_el else "Liên hệ"
            
            img_el = page.query_selector(".box-gallery-top .swiper-slide img")
            image_url = ""
            if img_el:
...
if __name__ == "__main__":
    ...
    print("=== BẮT ĐẦU TEST CLICKBUY SCRAPER ===")
    
    with BrowserManager(headless=False) as browser_manager:
        try:
            scraper = ClickbuyScraper(browser_manager=browser_manager)
            ...
            output_file = "clickbuy_results.json"
